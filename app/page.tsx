export default function Page() {
  const tools = [
    { title: "AI 知识点提取", desc: "上传 PDF 或图片，AI 自动为你提炼核心考点，生成复习讲义。", status: "运行中", color: "from-orange-400 to-orange-600", active: true },
    { title: "重生模拟器", desc: "回到高三，在“时空矩阵”中体验完全不同的人生选择。", status: "开发中", color: "from-slate-400 to-slate-500", active: false },
    { title: "德育积分榜", desc: "记录每一分成长，量化你的校园表现。", status: "预告", color: "from-slate-400 to-slate-500", active: false },
  ];
          ...
          {tools.map((tool, i) => (
            <div key={i} className="group relative bg-white rounded-3xl p-8 shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              <div className="flex justify-between items-start mb-6">
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${tool.color} shadow-lg shadow-orange-100`} />
              ...
              {tool.active ? (
                <button className="w-full py-4 bg-gray-900 text-white rounded-2xl font-bold hover:bg-orange-600 transition-colors">
                  立即进入 →
                </button>
              ) : (
